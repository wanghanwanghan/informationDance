...
class FrontEndUserApproximateEnterpriseModel extends ModelBase
{
    protected $tableName = 'approximateenterprise_';
    protected $autoTimeStamp = false;

    function addSuffix(int $uid): FrontEndUserApproximateEnterpriseModel
    {
        $suffix = $uid % 3;
    ...
    public  function findAllByCondition($whereArr){
        $res =  FrontEndUserApproximateEnterpriseModel::create()
            ->where($whereArr)
            ->all();
        return $res;
    }
    ...
    public  function findByConditionWithCountInfo($whereArr,$page){
        $model = FrontEndUserApproximateEnterpriseModel::create()
            ->where($whereArr)
            ->page($page)
            ->order('id', 'DESC')
            ->withTotalCount();
    ...
    public  function findByConditionV2($uid,$whereArr,$page,$size){
        $model = FrontEndUserApproximateEnterpriseModel::create()->addSuffix($uid);
    ...
    public  function setData($id,$field,$value){
        $info = FrontEndUserApproximateEnterpriseModel::findById($id);
        return $info->update([
            "$field" => $value,
        ]);
    }

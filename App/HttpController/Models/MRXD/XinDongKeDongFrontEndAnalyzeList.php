...
class XinDongKeDongFrontEndAnalyzeList extends ModelBase
{
    ...
    static  function  addRecordV2($info){
        ...
        return XinDongKeDongFrontEndAnalyzeList::addRecord(
            $info
        );
    }
    ...
    public static function addRecord($requestData){
        try {
           $res =  XinDongKeDongFrontEndAnalyzeList::create()->data([
    ...
    public static function searchFromEs($whereArr,$page,$limit){
        ...
        $model = XinDongKeDongFrontEndAnalyzeList::create();
    ...
    public static function findAllByCondition($whereArr){
        $res =  XinDongKeDongFrontEndAnalyzeList::create()
            ->where($whereArr)
            ->all();
        return $res;
    }
    ...
    public static function setTouchTime($id,$touchTime){
        $info = XinDongKeDongFrontEndAnalyzeList::findById($id);
        ...
        foreach ($records as $record){
            ...
            $res = self::addRecordV2(
                [
                    ...
                    'is_del' => XinDongKeDongFrontEndAnalyzeList::$state_ok,
                    'status' => XinDongKeDongFrontEndAnalyzeList::$status_init,
                    'name' => $paramsArr['name']?:'',
                    'ent_name' => $entName?:'',
    ...
    static function extractFeature($userId,$returnRaw =false){
        ...
        $res = [];
        $lists = XinDongKeDongFrontEndAnalyzeList::findAllByUserId($userId);
...
//            );
    ...
    static function extractFeatureV2($userId, $returnRaw =false, $retrunAllData = false){
        ...
        $res = [];
        $lists = XinDongKeDongFrontEndAnalyzeList::findAllByUserIdV2($userId);
//        CommonService::getInstance()->log4PHP(
//            json_encode([
//                __CLASS__.__FUNCTION__ .__LINE__,
...
//        $newEntNamesArr = explode(',',$newEntNames);
        ...
        //当前所有
        $allLists = XinDongKeDongFrontEndAnalyzeList::findAllByUserId($userId);
        foreach ($allLists as $data){
            //如果被删除了的话
            if(!in_array($data['ent_name'],$newEntNamesArr)){
                XinDongKeDongFrontEndAnalyzeList::delRecord($data['id']);
            }
        }
        ...
        foreach ($newEntNamesArr as $newEntName){
            ...
            // 添加
            $id = XinDongKeDongFrontEndAnalyzeList::addRecordV2(
                [
                    'user_id' => $userId,
                    'is_del' => XinDongKeDongFrontEndAnalyzeList::$state_ok,
                    'status' => XinDongKeDongFrontEndAnalyzeList::$status_init,

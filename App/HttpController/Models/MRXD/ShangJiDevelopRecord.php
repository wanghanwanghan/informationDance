...
//商机联系人
class ShangJiDevelopRecord extends ModelBase
{

    protected $tableName = 'shang_ji_develop_record';
    ...
    static  function  addRecordV2($info){
        ...
        return ShangJiDevelopRecord::addRecord(
            $info
        );
    }
    ...
    public static function addRecord($requestData){
        try {
           $res =  ShangJiDevelopRecord::create()->data($requestData)->save();

        } catch (\Throwable $e) {
            return CommonService::getInstance()->log4PHP(
                json_encode([
                    __CLASS__.__FUNCTION__ .__LINE__,
                    'shang_ji_develop_record_入库失败'=>[
                        '参数' => $requestData,
                        '错误信息' => $e->getMessage(),
                    ]
    ...
    public static function findAllByCondition($whereArr){
        $res =  ShangJiDevelopRecord::create()
            ->where($whereArr)
            ->all();
        return $res;
    }
    ...
    public static function findByConditionWithCountInfo($whereArr,$page){
        $model = ShangJiDevelopRecord::create()
                ->where($whereArr)
                ->page($page)
                ->order('id', 'DESC')
                ->withTotalCount();
    ...
    public static function findByName($shang_ji_develop_record_ming_cheng){
        $res =  ShangJiDevelopRecord::create()
            ->where('shang_ji_develop_record_ming_cheng',$shang_ji_develop_record_ming_cheng)
            ->get();
        return $res;
    }

    public static function setData($id,$field,$value){
        $info = ShangJiDevelopRecord::findById($id);
        return $info->update([
            "$field" => $value,
        ]);
    }

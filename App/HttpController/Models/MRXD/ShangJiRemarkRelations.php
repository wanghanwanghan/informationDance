...
class ShangJiRemarkRelations extends ModelBase
{

    protected $tableName = 'shang_ji_remark_relations';
    ...
    static  function  addRecordV2($info){
        ...
        return ShangJiRemarkRelations::addRecord(
            $info
        );
    }
    ...
    public static function addRecord($requestData){
        try {
           $res =  ShangJiRemarkRelations::create()->data($requestData)->save();

        } catch (\Throwable $e) {
            return CommonService::getInstance()->log4PHP(
                json_encode([
                    __CLASS__.__FUNCTION__ .__LINE__,
                    'shang_ji_remark_relations_入库失败'=>[
                        '参数' => $requestData,
                        '错误信息' => $e->getMessage(),
                    ]
    ...
    public static function findByConditionWithCountInfo($whereArr,$page){
        $model = ShangJiRemarkRelations::create()
                ->where($whereArr)
                ->page($page)
                ->order('id', 'DESC')
                ->withTotalCount();
    ...
    public static function setData($id,$field,$value){
        $info = ShangJiRemarkRelations::findById($id);
        return $info->update([
            "$field" => $value,
        ]);
    }

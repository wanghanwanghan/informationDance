...
class CompanyRiskService extends ServiceBase
{
     ...
     function exprXDS($data)
    {
        ...
        $res['gongshang'] = (0.6 * $res['企业性质'] + 0.2 * $res['企业对外投资'] + 0.2 * $res['融资历史']) * 0.1;
        $fz['gongshang'] = (0.6 * $res['企业性质'] + 0.2 * $res['企业对外投资'] + 0.2 * $res['融资历史']) * 0.1;
        //==============================================================================================================
        //行政许可
        $res['行政许可'] = $this->xzxk($data['GetAdministrativeLicenseList']['total']);
        $res['xingzheng'] = 0.05 * $res['行政许可'];
        $fz['xingzheng'] = 0.05 * $res['行政许可'];
        //==============================================================================================================
        //专利
        $res['专利'] = $this->zl($data['PatentV4Search']['total']);
        //软件著作权
        $res['软件著作权'] = $this->rjzzq($data['SearchSoftwareCr']['total']);
        $res['chuangxinyujishu'] = (0.6 * $res['专利'] + 0.4 * $res['软件著作权']) * 0.1;
        $fz['chuangxinyujishu'] = (0.6 * $res['专利'] + 0.4 * $res['软件著作权']) * 0.1;
        //==============================================================================================================
        //近三年团队人数
        $res['近三年团队人数'] = $this->tdrs($data['itemInfo'], 'fz');
        //近两年团队人数
        $res['近两年团队人数'] = $this->rybh($data['itemInfo'], 'fz');
        $res['tuandui'] = (0.5 * $res['近三年团队人数'] + 0.5 * $res['近两年团队人数']) * 0.1;
        $fz['tuandui'] = (0.5 * $res['近三年团队人数'] + 0.5 * $res['近两年团队人数']) * 0.1;
        //==============================================================================================================
        //招投标
        $res['招投标'] = $this->ztb($data['TenderSearch']['total']);
        $res['jingyingxinxi'] = 0.05 * $res['招投标'];
        //==============================================================================================================
        //财务资产
        $res['财务资产'] = $this->cwzc($data['FinanceData']['data'], 'fz');
        $res['caiwu'] = ($res['财务资产'] [0] * 0.6 + $res['财务资产'] [1] * 0.4) * 0.6;
        $fz['caiwu'] = ($res['财务资产'] [0] * 0.6 + $res['财务资产'] [1] * 0.4) * 0.6;
        //==============================================================================================================
        //行业位置
        $res['行业位置']  = $this->hywz($data['FinanceData']['data'], $data['getRegisterInfo']);
        $res['hangyeweizhi'] = 0.02 * $res['行业位置'];
        $fz['hangyeweizhi'] = 0.02 * $res['行业位置'];
        //==============================================================================================================
        //企业变更信息
        $res['企业变更信息'] = $this->qybgxx($data['getRegisterChangeInfo']['total']);
        //经营异常
        $res['经营异常']  = $this->jyyc($data['GetOpException']['total']);
        $res['gongshang'] = (0.6 * $res['企业变更信息'] + 0.4 * $res['经营异常']) * 0.05;
        $fx['gongshang'] = (0.6 * $res['企业变更信息'] + 0.4 * $res['经营异常']) * 0.05;
        //==============================================================================================================
        //财务资产
        $res['财务资产'] = $this->cwzc($data['FinanceData']['data'], 'fx');
        $res['caiwu'] = ($res['财务资产'][0] * 0.5 + $res['财务资产'][1] * 0.5) * 0.6;
        $fx['caiwu'] = ($res['财务资产'][0] * 0.5 + $res['财务资产'][1] * 0.5) * 0.6;
        //==============================================================================================================
        //近三年团队人数
        $res['近三年团队人数'] = $this->tdrs($data['itemInfo'], 'fx');
        //近两年团队人数
        $res['近两年团队人数'] = $this->rybh($data['itemInfo'], 'fx');
        $res['tuandui'] = (0.3 * $res['近三年团队人数'] + 0.7 * $res['近两年团队人数']) * 0.18;
        $fx['tuandui'] = (0.3 * $res['近三年团队人数'] + 0.7 * $res['近两年团队人数']) * 0.18;
        //==============================================================================================================
        //裁判文书
        ...
        $res['sifa'] = 0.25 * (($res['裁判文书'] + $res['执行公告']) / 2);
        $fx['sifa'] = 0.25 * (($res['裁判文书'] + $res['执行公告']) / 2);
        //==============================================================================================================
        //涉税处罚公示
        ...
        //计算
        $res['shuiwu'] = 0.1 * (($res['涉税处罚公示'] + $res['税务非正常户公示'] + $res['欠税公告']) / 3);
        $fx['shuiwu'] = 0.1 * (($res['涉税处罚公示'] + $res['税务非正常户公示'] + $res['欠税公告']) / 3);
        //==============================================================================================================
        //行政处罚
        $res['行政处罚'] = $this->xzcf($data['GetAdministrativePenaltyList']['total']);
        $res['xingzheng'] = 0.02 * $res['行政处罚'];
        $fx['xingzheng'] = 0.02 * $res['行政处罚'];
        //==============================================================================================================
        //联合惩戒名单信息（暂无该字段接口，先以司法类中的失信公告代替）失信公告的数量
        $res['联合惩戒名单信息'] = $this->sxgg($data['shixin']);
        $res['gaofengxian'] = 0.4 * $res['联合惩戒名单信息'];
        ...
        $csp->add('GetAdministrativePenaltyList', function () {
            ...
            if (!empty($res)) {
                foreach ($res as &$one) {
                    ...
                    $detail = (new LongDunService())->setCheckRespFlag(true)->get($this->ldUrl . 'AdminPenaltyCheck/GetCreditDetail', $postData);

...
class XlsWriter {
    ...
    public function  SetOut($filename)
    {
    ...
    protected function initializeSheet($sheet_name, $col_widths=array(), $auto_filter=false, $freeze_rows=false, $freeze_columns=false )
    {
        ...
        $this->sheets[$sheet_name] = (object)array(
            ...
            'row_count' => 0,
            'file_writer' => new \model\XLSXWriter_BuffererWriter($sheet_filename),
            'columns' => array(),
            'merge_cells' => array(),
        ...
        $max_cell= \model\XlsWriter::xlsCell(self::EXCEL_2007_MAX_ROW, self::EXCEL_2007_MAX_COL);//XFE1048577
...
class XLSXWriter_BuffererWriter
{
    ...
    public function __construct($filename, $fd_fopen_flags='w', $check_utf8=false)
    {
        ...
        if ($this->fd===false) {
            XlsWriter::log("Unable to open $filename for writing.");
        }
    }
    ...
    protected function purge()
    {
        if ($this->fd) {
            if ($this->check_utf8 && !self::isValidUTF8($this->buffer)) {
                XlsWriter::log("Error, invalid UTF8 encoding detected.");

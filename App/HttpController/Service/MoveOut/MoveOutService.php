...
class MoveOutService extends ServiceBase
{
    ...
    function updateDatabase(): ?array
    {
        ...
        foreach ($list as $oneEnt) {
            ...
            $basicInfo = EntDbBasic::create()
                ->where('ENTNAME', "{$oneEnt->entName}%", 'LIKE')
                ->all();
            if (!empty($basicInfo)) {
            ...
            $invInfo = EntDbInv::create()
                ->where('INV', $oneEnt->entName)
                ->where('CONRATIO', '66', '>=')->all();
            if (!empty($invInfo)) {
            ...
            $check = MoveOutEntNameFinance::create()

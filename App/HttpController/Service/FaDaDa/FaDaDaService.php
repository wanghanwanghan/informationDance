...
class FaDaDaService extends ServiceBase
{
     ...
     * @throws \Throwable
     */
    function getAuthFileByFile(array $arr): array
    {
        $file = $arr['file'];
        CommonService::getInstance()->log4PHP([$arr],'info','getAuthFile$arr');
        //企业注册
        ...
        //模板上传
        $uploadTemplateErrorData = $this->checkRet($this->uploadTemplate($arr,$file));
        if(!empty($uploadTemplateErrorData)) return $uploadTemplateErrorData;
        //模板填充
        $fillTemplateErrorData = $this->checkRet($this->fillTemplate($arr));
        if(!empty($fillTemplateErrorData)) return $fillTemplateErrorData;
    ...
    {
        CommonService::getInstance()->log4PHP([$arr],'info','getAuthFileForAnt$arr');
        //企业注册
        list($ent_customer_id,$entCustomerErrorData) = $this->getEntCustomer($arr);
        if(!empty($entCustomerErrorData)) return $entCustomerErrorData;
        $arr['customer_id'] = $ent_customer_id;
...
//        法人哈希存证
        ...
        if(!empty($ExtsignAutoErrorData)) return $ExtsignAutoErrorData;
        //自动签署法人姓名
        $ExtsignAutoErrorData = $this->checkRet($this->getExtsignAutoByWord($arr,$people_customer_id,$personal_sign_id,'#法人盖章处#'));
        if(!empty($ExtsignAutoErrorData)) return $ExtsignAutoErrorData;
        //合同下载
        $pdf_path = $this->downLoadContract($arr);
        //数据入库

...
class jincai_shoudong extends AbstractProcess
{
    ...
    // 取进项
    private function getInPiao(int $maxId, string $nsrsbh): ?array
    {
        $sql = <<<EOF
select * from invoice_main_input where gfsh = '{$nsrsbh}' and id > {$maxId} limit 500;
EOF;
        return sqlRaw($sql, 'business_base');
    }

    // 取销项
    private function getOutPiao(int $maxId, string $nsrsbh): ?array
    {
        $sql = <<<EOF
select * from invoice_main_output where xfsh = '{$nsrsbh}' and id > {$maxId} limit 500;
EOF;
